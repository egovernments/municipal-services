...
public class PaymentUpdateService {
		...
		try {
			...
			for (PaymentDetail paymentDetail : paymentRequest.getPayment().getPaymentDetails()) {
				...
				if (paymentDetail.getBusinessService().equalsIgnoreCase(config.getReceiptBusinessservice())) {
					...
					sendNotification(waterConnectionRequest);
				}
			}
		} catch (Exception ex) {
			log.error("", ex);
	 ...
	 */
	public void sendNotification(WaterConnectionRequest request) {
		notificationService.process(request, config.getOnWaterUpdated());
	}
}
