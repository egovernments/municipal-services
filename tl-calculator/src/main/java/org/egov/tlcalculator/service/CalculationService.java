...
public class CalculationService {
    ...
    private EstimatesAndSlabs getTaxHeadEstimates(CalulationCriteria calulationCriteria, RequestInfo requestInfo,
        ...
        if (isOwnerBPL(calulationCriteria.getTradelicense())) {
            ...
            calulationCriteria.getTradelicense().getTradeLicenseDetail().setAdhocExemption(baseTax); // baseTax will be
                                                                                                     // exempted for
                                                                                                     // BPL.
    ...
    private EstimatesAndSlabs getBaseTax(CalulationCriteria calulationCriteria, RequestInfo requestInfo,
        ...
        if (null != calulationCriteria.getTradelicense().getValidFrom()
                && null != calulationCriteria.getTradelicense().getValidTo()) {
    ...
    /**
     * The final tax needs to be multiplied with no of years the license is valid
     * for. eg - (2018-2019) total * 1, (2018-2020) total * 2, (2018 - 2021) total *
     * 3 etc.
     * 
     * @param totalTax
     * @param tradeLicense
    ...
    private boolean isOwnerBPL(TradeLicense tradeLicense) {
        ...
        try {
            Map<String, Object> addtionalDetails = mapper
                    .convertValue(tradeLicense.getTradeLicenseDetail().getAdditionalDetail(), Map.class);
            if (addtionalDetails.get("economicStatus").toString()
                    .equals(TLCalculatorConstants.MDMS_BPL_ECONOMIC_STATUS_CODE))
                isOwnerBPL = true;
        } catch (Exception e) {
    ...
    private Map<String, BigDecimal> getFeesFromMDMS(Object mdmsData) {
        ...
        try {
            String appFeesPath = TLCalculatorConstants.MDMS_TAXHEAD_PATH.replace("{}",
                    config.getApplicationFeesTaxHead());
    ...
    private FeeAndBillingSlabIds getTradeUnitFeeAndBillingSlabIds(TradeLicense license,
        ...
        for (TradeUnit tradeUnit : tradeUnits) {
            if (tradeUnit.getActive()) {
                ...
                searchCriteria.setTenantId(license.getTenantId());
                searchCriteria.setStructureType(license.getTradeLicenseDetail().getStructureType());
                searchCriteria.setApplicationType(
                        (String) ((HashMap<String, Object>) license.getTradeLicenseDetail().getAdditionalDetail())
                                .get("applicationType"));
                searchCriteria.setLicenseType(license.getLicenseType().toString());
