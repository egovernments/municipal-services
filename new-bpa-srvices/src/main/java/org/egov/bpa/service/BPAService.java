...
public class BPAService {
	...
	@Autowired
	private LandValidator landValidator;
	...
	public LandInfo landCreate(@Valid LandRequest landRequest) {
...
/*
	...
	public LandInfo landUpdate(@Valid LandRequest landRequest) {
		...
		if (landInfo.getId() == null) {
			throw new CustomException("UPDATE ERROR", "Application Not found in the System" + landInfo);
		}
		...
		List<BPA> searchResult = getLandInfoWithOwnerInfo(landRequest);
		if (CollectionUtils.isEmpty(searchResult)) {
			throw new CustomException("UPDATE ERROR", "Failed to Update the Application");
		}

//		landRequest.getLandInfo().setAuditDetails(searchResult.get(0).getAuditDetails());
		enrichmentService.enrichLandUpdateRequest(landRequest, businessService);


			        actionValidator.validateUpdateRequest(landRequest, businessService);
		*/
		return landInfo;
	}

	private List<BPA> getLandInfoWithOwnerInfo(@Valid LandRequest landRequest) {
		// TODO Auto-generated method stub
		return null;
	}

	public LandInfo search(@Valid LandSearchCriteria criteria, RequestInfo requestInfo) {
		// TODO Auto-generated method stub
		LandInfo landInfo;
		bpaValidator.validateSearch(requestInfo, criteria);
		if (criteria.getMobileNumber() != null) {
			landInfo = getLandInfoFromMobileNumber(criteria, requestInfo);
		} else {
			List<String> roles = new ArrayList<>();
			for (Role role : requestInfo.getUserInfo().getRoles()) {
				roles.add(role.getCode());
			...
			landInfo = getLandInfoWithOwnerInfo(criteria, requestInfo);
		}
		return landInfo;
	}

	private LandInfo getLandInfoWithOwnerInfo(@Valid LandSearchCriteria criteria, RequestInfo requestInfo) {
		// TODO Auto-generated method stub
		LandInfo landInfo = repository.getLandInfoData(criteria);
		
		landInfo = enrichmentService.enrichLandInfoSearch(landInfo, criteria, requestInfo);
		return landInfo;
	}
	...
	public LandInfo landSearch(@Valid LandSearchCriteria criteria, RequestInfo requestInfo) {
		// TODO Auto-generated method stub
		return null;
	}
