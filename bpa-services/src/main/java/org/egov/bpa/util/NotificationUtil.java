...
public class NotificationUtil {
	...
	public String getCustomizedMsg(RequestInfo requestInfo, BPA bpa,
		...
		if (bpa.getStatus().toUpperCase().equals(BPAConstants.STATUS_REJECTED)) {
			messageTemplate = getMessageTemplate(BPAConstants.APP_REJECTED,
					localizationMessage);
			message = getEnrichedMessage(bpa, messageTemplate);
		} else {
			String ACTION_STATUS = bpa.getAction() + "_" + bpa.getStatus();
			switch (ACTION_STATUS) {

			case BPAConstants.ACTION_STATUS_INITIATED:
				messageTemplate = getMessageTemplate(BPAConstants.APP_CREATE,
						localizationMessage);
				message = getEnrichedMessage(bpa, messageTemplate);
				break;
			...
			case BPAConstants.ACTION_STATUS_PENDING_APPROVAL:
				messageTemplate = getMessageTemplate(BPAConstants.NOC_APPROVE,
						localizationMessage);
				message = getEnrichedMessage(bpa, messageTemplate);
				break;
			...
			case BPAConstants.ACTION_STATUS_APPROVED:
				messageTemplate = getMessageTemplate(
						BPAConstants.APPROVE_PERMIT_GENERATED,
						localizationMessage);
				message = getEnrichedMessage(bpa, messageTemplate);
				break;
	...
	/**
	 * Creates customized event message based on bpa
	 * 
	 * @param bpa
	 *            The bpa for which message is to be sent
	 * @param localizationMessage
	 *            The messages from localization
	 * @param RequestInfo
	 *            The common request info for bpa
	 * @return customized message based on bpa
	 */
	public String getEventsCustomizedMsg(RequestInfo requestInfo, BPA bpa,
		...
		String message = null, messageTemplate;
		if (bpa.getStatus().toUpperCase().equals(BPAConstants.STATUS_REJECTED)) {
			messageTemplate = getMessageTemplate(BPAConstants.M_APP_REJECTED,
					localizationMessage);
			message = getEnrichedMessage(bpa, messageTemplate);
		} else {
			String ACTION_STATUS = bpa.getAction() + "_" + bpa.getStatus();
			...
			case BPAConstants.ACTION_STATUS_INITIATED:
				messageTemplate = getMessageTemplate(BPAConstants.M_APP_CREATE,
						localizationMessage);
				message = getEnrichedMessage(bpa, messageTemplate);
				break;
			...
			case BPAConstants.ACTION_STATUS_PENDING_SANC_FEE:
				messageTemplate = getMessageTemplate(
						BPAConstants.M_PERMIT_FEE_GENERATED,
						localizationMessage);
				message = getPaymentMsg(requestInfo, bpa, messageTemplate);
				break;
			...
			case BPAConstants.ACTION_STATUS_APPROVED:
				messageTemplate = getMessageTemplate(
						BPAConstants.M_APPROVE_PERMIT_GENERATED,
						localizationMessage);
				message = getEnrichedMessage(bpa, messageTemplate);
				break;
	...
	/**
	 * Creates customized message for all the states except payments
	 * 
	 * @param bpa
	 *            tenantId of the bpa
	 * @param message
	 ...
	 */
	private String getEnrichedMessage(BPA bpa, String message) {
	...
	/**
	 * Creates customized message for payments
	 * 
	 * @param requestInfo
	 *            common requestinfo  of the bpa
	 * @param bpa
	 *            tenantId of the bpa
	 * @param message
	 *            Message from localization for initiate
