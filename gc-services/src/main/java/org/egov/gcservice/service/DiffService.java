...
public class DiffService {
	...
	public void checkDifferenceAndSendEditNotification(GarbageConnectionRequest request,
			GarbageConnection searchResult) {
		try {
			GarbageConnection updateConnection = request.getGarbageConnection();
	 ...
	 * 
	 * @param updateConnection - Sewerage GarbageConnection Object
	 * @param searchResult - Sewerage GarbageConnection Object
	...
	private List<String> getObjectsRemoved(GarbageConnection updateConnection, GarbageConnection searchResult) {
		...
		for (Object object : changes) {
			String className = object.getClass().toString()
					.substring(object.getClass().toString().lastIndexOf('.') + 1);
			if (!classRemoved.contains(className) && !GCConstants.IGNORE_CLASS_ADDED.contains(className))
