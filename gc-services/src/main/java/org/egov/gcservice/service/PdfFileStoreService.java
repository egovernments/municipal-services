...
import org.egov.gcservice.web.models.Calculation;
...
public class PdfFileStoreService {
	 ...
	 * Get fileStroe Id's
	 * 
	 * @param garbageConnectionRequest - Sewerage connection Request
	 * @param property - Property Object
	 * @param applicationKey - ApplicationKey
	 * @return file store id
	 */
	public String getFileStoreId(GarbageConnectionRequest garbageConnectionRequest, Property property,
		...
		try {
			...
			BigDecimal slaDays = workflowService.getSlaForState(config.getBusinessServiceValue(),
					...
					garbageConnectionRequest.getRequestInfo(), applicationStatus);
			sewerageObject.put(sla, slaDays.divide(BigDecimal.valueOf(GCConstants.DAYS_CONST)));
			...
			return getFileStoreIdFromPDFService(sewerageObject, garbageConnectionRequest.getRequestInfo(), tenantId,
					applicationKey);
		} catch (Exception ex) {
	...
	private String getFileStoreIdFromPDFService(JSONObject sewerageObject, RequestInfo requestInfo, String tenantId,
		...
		requestPayload.put(WaterConnectionReplacer, GarbageConnectionList);
		try {
	...
	public void process(GarbageConnectionRequest garbageConnectionRequest, String topic) {
		...
		if (garbageConnectionRequest.getGarbageConnection().getStatus()
		...
		sewerageDao.saveFileStoreIds(garbageConnectionRequest);
	}
}
