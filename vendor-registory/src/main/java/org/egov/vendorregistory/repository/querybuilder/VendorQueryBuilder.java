package org.egov.vendorregistory.repository.querybuilder;
...
public class VendorQueryBuilder {
	...
	private static final String Query = "select vendor.*,vendor_address.*,vendor_driver.*,vendor_vehicle.*, vendor.id as vendor_id,"
			...
			+ "  vendor.lastmodifiedtime as vendor_lastmodifiedtime,"
	...
	public String getVendorSearchQuery(VendorSearchCriteria criteria, List<Object> preparedStmtList) {
		...
		if (criteria.getTenantId() != null) {
			...
			List<String> owner_name = criteria.getOwnerName();
			if (!CollectionUtils.isEmpty(owner_name)) {
				addClauseIfRequired(preparedStmtList, builder);
				builder.append(" vendor.name IN (").append(createQuery(owner_name)).append(")");
				addToPreparedStatement(preparedStmtList, owner_name);
			...
			if(!CollectionUtils.isEmpty(ids)) {
				addClauseIfRequired(preparedStmtList, builder);
				builder.append(" vendor.vendor_id IN (").append(createQuery(ownerIds)).append(")");
				addToPreparedStatement(preparedStmtList, ownerIds);
			}
		}
