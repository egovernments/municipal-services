...
public class PlantMappingUtil {
	...
	public List<ModuleDetail> getFSMModuleRequest() {
		...
		// filter to only get code field from master data
				final String filterCode = "$.[?(@.active==true)].PlantCode";
//         final String activeFilter = "$.[?(@.active==true)]";
		// master details for FSM module
		List<MasterDetail> fsmMasterDtls = new ArrayList<>();

		fsmMasterDtls.add(MasterDetail.builder().name(PlantMappingConstants.MDMS_FSTP_PLANT_INFO).filter(filterCode).build());
		ModuleDetail fsmMasterMDtl = ModuleDetail.builder().masterDetails(fsmMasterDtls)
				.moduleName(PlantMappingConstants.FSM_MODULE_CODE).build();
