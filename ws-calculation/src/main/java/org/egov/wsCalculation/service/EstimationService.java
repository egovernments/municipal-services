...
public class EstimationService {
	...
	private List<TaxHeadEstimate> getEstimatesForTax(String assessmentYear, BigDecimal taxAmt,
		...
		estimates.add(
				TaxHeadEstimate.builder().taxHeadCode(PT_UNIT_USAGE_EXEMPTION).estimateAmount(usageExemption).build());
		...
		if (usePBFirecessLogic) {
			...
			estimates.add(TaxHeadEstimate.builder().taxHeadCode(PT_FIRE_CESS).estimateAmount(fireCess.setScale(2, 2))
					.build());
		} else {
		...
		estimates.add(TaxHeadEstimate.builder().taxHeadCode(PT_CANCER_CESS).estimateAmount(cancerCess.setScale(2, 2))
				.build());

		List<Receipt> receipts = Collections.emptyList();

		if (property.getPropertyId() != null) {
			rcptService.getReceiptsFromPropertyAndFY(assessmentYear, property.getTenantId(), property.getPropertyId(),
					requestInfoWrapper);
		}
