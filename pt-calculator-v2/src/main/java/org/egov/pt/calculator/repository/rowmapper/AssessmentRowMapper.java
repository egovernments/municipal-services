...
import org.egov.pt.calculator.web.models.Status;
...
public class AssessmentRowMapper implements ResultSetExtractor<List<Assessment>> {
	...
	public List<Assessment> extractData(ResultSet rs) throws SQLException, DataAccessException {
		...
		while (rs.next()) {
			...
			if (null == assessment) {
				...
				DocumentV2 document = getDocument(rs);
				if (document != null)
	...
	private DocumentV2 getDocument(ResultSet rs) throws SQLException {
		...
		return DocumentV2.builder().id(rs.getString("doc_id")).documentType(rs.getString("doc_documenttype"))
				.documentUid(rs.getString("doc_documentuid")).fileStoreId(rs.getString("doc_filestoreid"))
